...
?> </a></div>
 <table class="table table-striped table-bordered bootstrap-datatable ">
  ...
  <label class="control-label" for="inputSuccess1"  <select name="cat_id"> 
        <?php
        foreach($list_users as $k=>$v){
            ?><option  <?php echo @$detail[0]->id==$v->id?"selected":"";  ?> value="<?php echo $v->id ?>"><?php echo $v->users ?></option><?php
        }
        ?> 
    </select>      >Nhập  username</label>
